...
function recycle(arr) {
  const res = { 'organic': [],'paper': [], 'plastic': [], 'glass': [] };
  for (let key of arr) {
    res[key.material].push(key.type);
    if (key.secondMaterial)  res[key.secondMaterial].push(key.type);
  }
  return [res['paper'], res['glass'], res['organic'], res['plastic']];
}
...
const recycle = objects => ['paper', 'glass', 'organic', 'plastic'].map(m => objects.filter(o => o.material == m || o.secondMaterial == m).map(o => o.type));
